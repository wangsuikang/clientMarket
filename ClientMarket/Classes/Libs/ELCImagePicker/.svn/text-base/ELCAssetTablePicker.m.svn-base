...
//        [self.navigationItem setTitle:NSLocalizedString(@"Loading...", nil)];
        [self addBarButtonItemWithTitle:nil imageName:@"back" frame: CGRectMake(0, 0, 20, 22)target:self action:@selector(backAction) isLeft:YES isCircle:YES];
        
        [self addBarButtonItemWithTitle:@"确定" imageName:@"" frame: CGRectMake(SCREEN_WIDTH-60, 0, 60, 44)target:self action:@selector(doneAction:) isLeft:NO isCircle:YES];
    }
...
- (void)preparePhotos
{
    @autoreleasepool {
        ...
        dispatch_sync(dispatch_get_main_queue(), ^{
            ...
            [self.navigationItem setTitle:self.isVideo ? NSLocalizedString(@"图片", nil) : NSLocalizedString(@"视频", nil)];
        });
    }
}
